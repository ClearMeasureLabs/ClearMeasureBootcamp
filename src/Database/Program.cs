...
namespace ClearMeasure.Bootcamp.Database
{
    internal class Program
    {
        private static string databaseName = GetDatabaseName();
        private static string DbServer = GetDatabaseServer();
            ...
            while (selector != maxMenuItems)
            {
                ...
                if (good)
                {
                    ...
                    switch (selector)
                    {
                        ...
                        case 4:
                        case 5:
                            string cmdArguments = string.Format("{0} {1} {2} {3}", GetVerbForCase(selector), DbServer,
                                databaseName, scriptspath);
        ...
        {
            string result;
            var dbServer = Environment.GetEnvironmentVariable("dbServer", EnvironmentVariableTarget.User);
            ...
            {
                result = ConfigurationManager.AppSettings["DatabaseServer"];
